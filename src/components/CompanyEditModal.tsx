...
const ICON_OPTIONS = [
  ...
  { value: 'toy', label: 'Ursinho', Icon: ToyBrick },
  { value: 'hotdog', label: 'Cachorro Quente', Icon: UtensilsCrossed },
  { value: 'wrench', label: 'Chave de Fenda', Icon: Wrench },
...
const getIconLabel = (iconValue: string) => {
  const found = ICON_OPTIONS.find(opt => opt.value === iconValue);
  return found ? found.label : 'Tesoura';
};
...
export const CompanyEditModal = ({
  ...
  const CurrentIcon = getIconComponent(companyDataForm.icon);
  ...
  return (
    <>
          ...
          <div className="flex-1 overflow-auto px-4 py-3">
            {activeTab === 'dados' ? (
              ...
              <div className="flex flex-col gap-4">
                {/* Icon Selector - Single icon with + button */}
                <div className="space-y-2">
                  <Label className="text-xs text-gray-400">Ícone da Empresa</Label>
                  <div className="flex items-center gap-3">
                    {/* Current selected icon */}
                    <div className="flex flex-col items-center gap-1 p-3 rounded-xl bg-orange-500 text-white">
                      <CurrentIcon className="w-6 h-6" />
                      <span className="text-[10px]">{getIconLabel(companyDataForm.icon)}</span>
                    </div>
                    ...
                    <Button
                      ...
                      className="w-14 h-14 rounded-xl border-dashed border-2 border-white/20 bg-transparent hover:bg-white/10 hover:border-orange-500/50"
                    >
      ...
      <Dialog open={showIconPicker} onOpenChange={setShowIconPicker}>
        <DialogContent className="sm:max-w-sm bg-[#1a1a1a] border-white/10 text-white p-0 [&>button]:w-8 [&>button]:h-8">
          <DialogHeader className="px-4 pt-4 pb-2">
            <DialogTitle className="text-white text-center">Escolha um Ícone</DialogTitle>
          </DialogHeader>
            ...
            <div className="grid grid-cols-4 gap-3">
              {ICON_OPTIONS.map(({ value, label, Icon }) => (
                <button
                  ...
                  type="button"
                  onClick={() => {
                    setCompanyDataForm({...companyDataForm, icon: value});
